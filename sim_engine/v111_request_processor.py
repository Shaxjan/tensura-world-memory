...
def _decode_request(req: dict[str, Any], pointer: dict[str, Any], session: dict[str, Any]) -> tuple[int, str, str, dict[str, Any], str]:
    fmt = req.get("format")
    ...
    if fmt == LEGACY_REQUEST_FORMAT:
        ...
        return seq, event_key, event_type, payload, "legacy_explicit_seq"
    ...
    if not isinstance(request_id, str) or not request_id or len(request_id) > 160:
        raise ValueError("fast request_id required")
    if "seq" in req:
    ...
    return next_seq, event_key, event_type, payload, "fast_auto_seq"
...
def process_request(repo_root: str | Path, request_path: str | Path) -> dict[str, Any]:
    ...
    if pointer.get("engine_version") != "1.0.11":
        raise RuntimeError("v1.0.11 request processor requires engine 1.0.11")
    ...
    with tempfile.TemporaryDirectory() as td:
        world, loaded, meta = load_repository_runtime_v111(root, Path(td) / "live.db")
        try:
