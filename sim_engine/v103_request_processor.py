...
def process_request(repo_root: str | Path, request_path: str | Path) -> dict[str, Any]:
    ...
    if pointer.get("engine_version") != "1.0.3":
        raise RuntimeError("v1.0.3 request processor requires engine 1.0.3")
    ...
    with tempfile.TemporaryDirectory() as td:
        world, loaded_pointer, meta = load_repository_runtime_v103(root, Path(td) / "live.db")
        try:
            ...
            session_state = world.build_session_state_v103(
                journal_seq=seq, head_state_hash=entry["after_hash"], last_event=entry)
        finally:
            world.close()
    ...
    return {"ok": True, "seq": seq, "event_key": event_key, "event_type": event_type,
            "journal_path": str(journal_path.relative_to(root)), "session_state": "runtime/session_state.json",
