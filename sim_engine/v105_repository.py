...
def validate_runtime_pointer_v105(pointer: dict) -> None:
...
def load_repository_runtime_v105(repo_root: str | Path, db_path: str | Path):
    root = Path(repo_root).resolve()
    pointer = json.loads((root / "runtime/runtime_state.json").read_text(encoding="utf-8"))
    validate_runtime_pointer_v105(pointer)
