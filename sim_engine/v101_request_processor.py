...
def process_request(repo_root: str | Path, request_path: str | Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory() as td:
        world, loaded_pointer, meta = load_repository_runtime_v101(root, Path(td) / "live.db")
        try:
    ...
    return {
        ...
        "result_status": (entry.get("result") or {}).get("status") or (entry.get("result") or {}).get("result", {}).get("status"),
