...
def process_request(repo_root: str | Path, request_path: str | Path) -> dict[str, Any]:
    ...
    if pointer.get("engine_version") != "1.0.8": raise RuntimeError("v1.0.8 request processor requires engine 1.0.8")
    ...
    with tempfile.TemporaryDirectory() as td:
        world, loaded_pointer, meta = load_repository_runtime_v108(root, Path(td) / "live.db")
        try:
            if loaded_pointer["head_state_hash"] != pointer["head_state_hash"]: raise RuntimeError("pointer changed during load")
            ...
            session_state = world.build_session_state_v108(journal_seq=seq, head_state_hash=entry["after_hash"], last_event=entry)
        finally:
            world.close()

    journal_path.parent.mkdir(parents=True, exist_ok=True)
    journal_path.write_text(json.dumps(entry, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    pointer.update({"engine_version":"1.0.8","journal_seq":seq,"head_state_hash":entry["after_hash"],
    ...
    return {"ok":True,"seq":seq,"event_key":event_key,"journal_path":str(journal_path.relative_to(root)),
            "before_hash":entry["before_hash"],"after_hash":entry["after_hash"],
            "result_status":(entry.get("result") or {}).get("status"),"replay_before":meta["replay"].get("replayed",0)}


def main():
    ap=argparse.ArgumentParser(); ap.add_argument("--repo-root",default=".."); ap.add_argument("--request",required=True); ap.add_argument("--out")
    args=ap.parse_args(); result=process_request(args.repo_root,args.request); text=json.dumps(result,ensure_ascii=False,indent=2)
